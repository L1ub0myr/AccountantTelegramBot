...
class DataBase:
    def __init__(self, user_id: int, name_database: str, name_table: str):
        ...
        self._connect = sqlite3.connect(f'./models/files_database/{name_database}.db')
    ...
    def _check_user_exists(self):
        with self._connect:
            for i in self._cursor.execute(f"""SELECT user_id FROM {self._name_table}""").fetchall():
    ...
    def _add_user(self):
        with self._connect:
            self._cursor.execute(f"""INSERT INTO {self._name_table}(user_id) VALUES(?)""", (self._user_id,))
